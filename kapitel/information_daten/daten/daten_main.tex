...
Computer verarbeiten \textbf{Daten}. Genauer gesagt: Der Computer arbeitet mit elektronischen Daten\footnote{Die Abkürzung \ac{EDV} ist im deutschsprachigen Raum ein gängiger Begriff.}.

\begin{definition}[Daten]
Messwerte, Fakten, Gegebenheiten oder Zeitpunkte bezeichnen wir als Daten. Sie bestehen aus Ziffern, Buchstaben und Sonderzeichen oder werden durch eine Funktion beschrieben und sind meist mit einer Einheit versehen \cite{cite-donze-introduction}.
\end{definition}

\begin{example}
Es gibt zahlreiche Beispiele für Daten. Wir zeigen hier ausgewählte Beispiele.
\begin{multicols}{3}
\begin{itemize}
...
\item Alice
\item 4. Mai 1977
\item 1875 positive \ac{PCR}-Tests
\end{itemize}
\end{multicols}
\end{example}
...
\begin{figure}[htb]
\centering
\includegraphics[height=7.5cm]{wissenspyramide.jpeg}
...
Die Ziffern $0$ und $1$ werden binäre Ziffern (eng. binary digit) oder kurz \textbf{Bit} genannt. Ein Bit kann also zwei verschiedene Werte annehmen. Eine Folge von Bits bezeichnen wir somit als Binärdaten.
\end{definition}
